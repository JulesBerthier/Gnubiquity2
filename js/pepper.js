var pepper_ip_0 = '193.48.125.58';
var host = pepper_ip_0;

function connected() { //fonction pour se connecter au robot
  console.log('connecté à Pepper');
}

function disconnected() { //fonction pour se déconnecter
  console.log('déconnecté de Pepper');
}
...
QiSession(function(session) {
  	...
  	//On commence par créer le joystick qui permettra de faire bouger la tête
  	$("#nipple_0_0").append(function(){
  		var options={
  			zone: document.getElementById('nipple_0_0'),
  			mode: 'static', //peut aussi être dynamic ou semi
  	...
  	//ensuite on définit les fonction que le joystick va nous permettre d'effectuer
  		...
  		manager.on('dir:up',function(evt,nipple){
  			session.service("ALMotion").then(function(motionProxy){
  	...
  	session.service("ALAutonomousLife").then(function(autonomousLifeProxy){
  	...
  	$("#attention").click(function(){
  		session.service("ALMotion").then(function(motionProxy){
  			...
  			setTimeout(function(){
  				session.service("ALRobotPosture").then(function(robotPostureProxy){
...
//Ici on définit les fonction des boutons situés dans le menu déroulant "posture du robot"
	...
	//Au clic sur les boutons du menu déroulant, le robot change de posture parmis les postures prédéfinies
  	$("#Sit").click(function(){
  		session.service("ALRobotPosture").then(function(postureProxy){
